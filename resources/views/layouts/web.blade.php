    ...
    <div id="spark-app" v-cloak>
        ...
        @section('footer')
            <footer class="footer footer-bar">
                <div class="container">
                    <p class="mb-0">© {{ date('Y') }} <a href="{{ config('app.url') }}">{{ config('app.name') }}</a> by <a href="{{ config('base.copyrigth_url') }}" target="_blank" rel="noopener">{{ str_replace('https://', '', config('base.copyrigth_url')) }}</a>. {{__('All rights reserved.')}}</p>
                </div>
            </footer>
